...
import ve.zoonosis.vistas.modulos.estadistica.jornada.especies.municipio.JornadaEspecieMensualPorMunicipio;
 ...
 * @author Kelvin López
 */
public class JornadaEspecieMensualPorMunicipioController extends JornadaEspecieMensualPorMunicipio {

    private static final Logger LOG = Logger.getLogger(NuevoCasoController.class);
    private RequestBuilder rb;

    public JornadaEspecieMensualPorMunicipioController() {
        inicializar();
    ...
    private void obtenerEstadistica() {
        ...
        try {
           ...
           // final String fecha = numeroMes+"-"+numeroAño;
            ...
            rb = new RequestBuilder("services/funcionales/EspecieWs/ObtenerListaEspeciesPorMesDeJornadaPorMunicipio.php",
            ...
            java.util.logging.Logger.getLogger(JornadaAnimalDiarioPorMunicipioController.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private List<HashMap> sumarRepetidos(final List<HashMap> valores) {
        ...
        for (int i = 0; i < valores.size(); i++) {
            System.out.println("Turno de "+valores.get(i).get("nombre").toString());
            esRepetido = false;
            final int index = i;
            final int sumf;
            int sumt = 0;
            for (HashMap sumado : sumados) {
                System.out.println("Verificando "+sumado.get("nombre").toString()+" Con "+valores.get(i).get("nombre").toString());
